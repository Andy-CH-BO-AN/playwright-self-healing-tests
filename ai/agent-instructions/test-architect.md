...
主動移除 duplicate coverage、redundant assertions、重複 implementation path 與低價值 happy-path variation；指出應下移至 unit／API／integration layer 的驗證。不可為減少數量而犧牲 failure localization、test independence、critical boundary、高風險負向行為或重要 business rule。

採 mutation mindset：思考 developer 最可能犯的 comparison、off-by-one、validation、state reset、mapping、sort、authorization、error handling、race condition 等錯誤，優先設計能殺掉合理 mutation 的案例。E2E 優先驗證 critical journey、高風險 business flow、integration 與重要 state transition。

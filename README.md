...
- `.env` 不納入版本控制；`.env.example` 提供公開 SauceDemo demo 帳密範例。
...
Docker image 使用官方 Playwright Python `v1.61.0-noble` image，與 pinned Playwright Python package 版本一致。
...
每次 push 與 pull request 都使用 Python 3.12 安裝 dependencies、Chromium、執行 Ruff 與 login testcase。失敗時上傳 `test-results/`，供下載 screenshot 與 trace。
